...
local STR = {
  zh = {
    key_hint = " 关闭(q)  刷新(r)  激活(Enter)  目录(t)  顶(go)  折(za)  折全(zM)  展全(zR)  帮助(?)  中英(L)  图(gi)  复制(c)  源(gs)  │ mdview ",
    contents = "◆ 目录",
    copy = "[复制]",
    copied = "[已复制]",
    help_title = " 帮助 ",
    help_lines = {
      ...
      "  C-左键     同上（Ctrl+鼠标左键）",
      "  非光标行   ![alt](url) 显示为 🖼 name（alt 空 → image）",
      "  zc / zo    收起 / 展开光标处折叠（标题 / details）",
      "  za         切换光标处折叠",
      "  zM / zR    收起 / 展开全部折叠",
      "  details    源码默认收起正文（open 属性则展开）",
      "  \"+p / C-S-v     粘贴：剪贴板有图 → images/yyyyMMddHHmmss.png + 链接",
      "  :MdViewPasteImage  仅粘贴剪贴板图片（无图则提示）",
      "",
      "  gt / gT    切换 tab（系统默认）",
      "",
      "  q / Esc    关闭本窗口",
    ...
    paste_need_file = "mdview: 请先保存 markdown 文件再粘贴图片",
    paste_no_python = "mdview: 未找到 python（粘贴图片需要 Python + Pillow）",
    paste_need_pillow = "mdview: 粘贴图片需要 Pillow（pip install Pillow）",
    paste_no_image = "mdview: 剪贴板中没有图片",
    paste_mkdir_fail = "mdview: 无法创建图片目录: ",
    paste_fail = "mdview: 保存剪贴板图片失败: ",
    paste_ok = "mdview: 已保存并插入 ",
    source_fold_lines = "… (%d 行)",
  },
  en = {
    key_hint = " Close(q)  Refresh(r)  Enter  TOC(t)  Top(go)  Fold(za)  FoldAll(zM)  OpenAll(zR)  Help(?)  Lang(L)  Img(gi)  Copy(c)  Src(gs)  │ mdview ",
    contents = "◆ Contents",
    copy = "[Copy]",
    copied = "[Copied]",
    help_title = " Help ",
    help_lines = {
      ...
      "  q          Close preview",
      "  r          Refresh preview",
      "  <CR>       Activate: TOC / heading ▸ fold / code fold / image / link",
      ...
      "  za         Toggle smallest fold under cursor",
      ...
      "  other lines  ![alt](url) shows as 🖼 name (empty alt → image)",
      "  zc / zo    Close / open fold (headings / details)",
      "  za         Toggle fold under cursor",
      "  zM / zR    Collapse / expand all folds",
      "  details    Source body collapsed by default (unless open)",
      "  \"+p / C-S-v     Paste: clipboard image → images/yyyyMMddHHmmss.png + link",
      "  :MdViewPasteImage  Paste clipboard image only (warn if none)",
      "",
    ...
    link_nf = "mdview: link target not found: ",
    ...
    paste_need_file = "mdview: save the markdown file before pasting an image",
    paste_no_python = "mdview: python not found (paste image needs Python + Pillow)",
    paste_need_pillow = "mdview: Pillow required for paste image (pip install Pillow)",
    paste_no_image = "mdview: clipboard has no image",
    paste_mkdir_fail = "mdview: cannot create image directory: ",
    paste_fail = "mdview: failed to save clipboard image: ",
    paste_ok = "mdview: saved and inserted ",
    source_fold_lines = "… (%d lines)",
  },
}
