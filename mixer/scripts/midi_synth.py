# -*- coding: utf-8 -*-
"""mixer.nvim — Windows winmm.dll (MCI) MIDI player daemon (JSON lines).
...
def _preset_cache_dir() -> Path:
    ...
    d = Path(base) / "nvimplugins-mixer-presets"
...
def main() -> int:
    if sys.platform != "win32":
        emit({"ok": False, "error": "mixer winmm backend requires Windows"})
        return 1
    try:
        eng = MixerEngine()
    except Exception as e:
        emit({"ok": False, "error": f"mixer init failed: {e}"})
        return 1
