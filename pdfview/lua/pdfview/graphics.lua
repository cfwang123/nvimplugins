...
local function place_visible_slice(place, topline, botline)
  local a = tonumber(place.line) or 1
  local b = tonumber(place.line_end)
  local rows = tonumber(place.rows)
  if not b then
    b = a + math.max(1, rows or 1) - 1
  end
  local full = math.max(1, rows or (b - a + 1))
  topline = tonumber(topline) or 1
  botline = tonumber(botline) or topline
...
local function send_iterm(b64, cols, rows, row, col)
  local name_b64 = b64encode("pdfview.png")
...
local function paint_one(ov, place)
  if not place or not place.b64 then
    return
  end
  -- float 全窗叠层无 buffer 行坐标，由 paint_fn 绘制，勿走预览路径
  if ov and ov.float_mode then
    return
  end
  if not ov.win or not vim.api.nvim_win_is_valid(ov.win) or vim.api.nvim_win_get_buf(ov.win) ~= ov.buf then
    return
  end

  local top, bot = win_view(ov.win)
  -- place.line / line_end 缺失时（异常数据）直接跳过，避免 nil 比较
  local pline = tonumber(place.line)
  local pline_end = tonumber(place.line_end) or pline
  if not pline or not pline_end then
    return
  end
  place.line = pline
  place.line_end = pline_end

  if pline_end < top or pline > bot then
  ...
  local full_rows = math.max(1, place.rows or (pline_end - pline + 1))
  ...
  local vis_start = math.max(pline, top)
...
function M._repaint_buf(buf)
  ...
  -- float 模式：用 attach_float 挂上的 paint_fn，不要走 paint_one（无 line 坐标）
  if ov.float_mode then
    if type(ov.paint_fn) == "function" then
      pcall(ov.paint_fn)
      return true
    end
    return false
...
function M.attach_preview(opts)
  ...
  local aug = vim.api.nvim_create_augroup("pdfviewGfx_" .. buf, { clear = true })
  ov.aug = aug
...
function M.attach_float(opts)
  ...
  if not imgcfg then
    pcall(function()
      imgcfg = require("pdfview.config").get().image or {}
    end)
    imgcfg = imgcfg or {}
  end
  ...
  local ov = {
    ...
    float_mode = true,
    debug_once = false,
    paint_fn = nil, ---@type function|nil
  }
  overlays[buf] = ov
  ...
  local function paint_float()
    ...
    if not screen_row or not screen_col or screen_row < 1 or screen_col < 1 then
      return
    end
    local p = places[1]
    if not p then
    ...
    cw = math.max(1, cw or p.cols or 1)
    ch = math.max(1, ch or p.rows or 1)
    if math.abs(cw - (p.cols or 0)) > 0 or math.abs(ch - (p.rows or 0)) > 0 then
    ...
    if imgcfg.hd_debug and not ov.debug_once then
      ...
      vim.notify(
        string.format(
          "pdfview float_hd @%d,%d size %dx%d (draw_rows=%d crop_top) border=%d",
          screen_row,
          screen_col,
          p.cols,
          p.rows,
  ...
  local aug = vim.api.nvim_create_augroup("pdfviewFloatGfx_" .. buf, { clear = true })
  ov.aug = aug
