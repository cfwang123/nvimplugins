...
local STR = {
  zh = {
    ...
    mine_ready = " %s %dx%d  雷:%d  | 左键开 右键旗  中键/双键:弦开  1/2/3难度  L语言  q退出",
    mine_playing = " %s  剩余雷:%d  用时:%ds  | 左键开 右键旗  L语言  q退出",
    mine_footer = " [%s] [%s] [%s]   [%s]  [%s]",
    mine_help = "扫雷\n左键:开格  右键:插旗  中键:弦开\n1/2/3:初/中/高  r:重开  L:中英文  hjkl:选择  空格:开  m:旗  c:弦  q:退出",

    -- sokoban
    sokoban_title = " 推箱子  关卡 %d/%d  %s  步数:%d  %s",
    sokoban_cleared = "★ 过关！",
    sokoban_help = " hjkl/方向键移动  z撤销  r重开  n/p下/上关  g跳关  L中英文  q退出 ",
    ...
    tf_help = "  公式行输入  Enter 判定  出错后 Space 清空  r 新牌  h 答案  L 中英文  q 退出  ",
    tf_help_box = "24点\n在「公式>」后直接输入算式，Enter 判定\n出错后 Space 清空输入；r 新牌会清空公式\nA=1 J=11 Q=12 K=13；四则运算 + - * / ()\ni 编辑  r 新牌  h 答案  L 中英文  点击底栏按钮也可切换  q 退出\n例: (8/(3-8/3))",
    ...
    -- tetris
    tetris_help = "  hjkl移动 空格硬降 z/x转  p暂停  r重开  v人机  L语言  q退出  ",
    tetris_help_vs = "  hjkl移动 空格硬降 z/x转  p暂停  r重开  m单人  L语言  q退出 | 消越多罚越重 ",
    tetris_over = "  ★ GAME OVER  按 r 重开  v 人机  L 语言  q 退出",
    tetris_you_win = "  你赢了！按 r 再战  m 单人  L 语言  q 退出",
    tetris_ai_win = "  电脑获胜！按 r 再战  m 单人  L 语言  q 退出",
    tetris_draw = "  双双出局… 按 r 再战  L 语言  q 退出",
    tetris_help_box = "俄罗斯方块\n操作: hjkl/方向  空格硬降  z/x 旋转  p 暂停  r 重开  L 中英文  q 退出\nv 人机对战  m 单人\n对战: 惩罚=三角数(清行+特殊格)；对方落地后顶入垃圾\n特殊↓←↑→: 箭头=朝向；↓填下 ←→填侧；↑仅1格",
  ...
  en = {
    menu_title = "nvimgames",
    menu_hint = "1-4 select · L language · Esc quit",
    ...
    mine_ready = " %s %dx%d  mines:%d  | LMB open  RMB flag  MMB/chord  1/2/3 difficulty  L lang  q quit",
    mine_playing = " %s  left:%d  time:%ds  | LMB open  RMB flag  L lang  q quit",
    mine_footer = " [%s] [%s] [%s]   [%s]  [%s]",
    mine_help = "Minesweeper\nLMB:open  RMB:flag  MMB:chord\n1/2/3:difficulty  r:restart  L:language  hjkl  Space:open  m:flag  c:chord  q:quit",

    sokoban_title = " Sokoban  Lv %d/%d  %s  steps:%d  %s",
    sokoban_cleared = "★ Cleared!",
    sokoban_help = " hjkl/arrows move  z undo  r restart  n/p next/prev  g jump  L lang  q quit ",
    ...
    tf_help = "  type formula  Enter check  Space clear after error  r new  h hint  L lang  q quit  ",
    tf_help_box = "24 Points\nType formula after expr> , Enter to check\nSpace clears after error; r deals new cards\nA=1 J=11 Q=12 K=13; ops + - * / ()\ni edit  r new  h hint  L language (or click bottom button)  q quit\ne.g. (8/(3-8/3))",
    ...
    tetris_help = "  hjkl move  Space hard-drop  z/x rotate  p pause  r restart  v vs-AI  L lang  q quit  ",
    tetris_help_vs = "  hjkl move  Space hard-drop  z/x rotate  p pause  r restart  m solo  L lang  q quit | more clears=more garbage ",
    tetris_over = "  ★ GAME OVER  r restart  v vs-AI  L lang  q quit",
    tetris_you_win = "  You win!  r again  m solo  L lang  q quit",
    tetris_ai_win = "  AI wins!  r again  m solo  L lang  q quit",
    tetris_draw = "  Both out…  r again  L lang  q quit",
    tetris_help_box = "Tetris\nhjkl/arrows  Space hard-drop  z/x rotate  p pause  r restart  L language  q quit\nv vs-AI  m solo\nvs: triangular garbage (lines+special cells)\nspecial ↓←↑→: arrow=dir; ↓ fill down; ←→ side; ↑ 1 cell only",
